...
void Player::sendCancelMessage(ReturnValue message) const
{
	switch(message)
	{
		...
		case RET_PLAYERISPZLOCKED:
			sendCancel("Você não pode entrar na área protegida após atacar outro jogador.");
			break;

		case RET_PLAYERISNOTINVITED:
			sendCancel("Você não foi convidado.");
			break;
...
void Player::addExperience(uint64_t exp)
{
	...
	if(prevLevel != level)
	{
		...
		char advMsg[60];
		sprintf(advMsg, "Você avançou do level %d até o level %d.", prevLevel, level);
		sendTextMessage(MSG_EVENT_ADVANCE, advMsg);
...
void Player::removeExperience(uint64_t exp, bool updateStats/* = true*/)
{
	...
	if(prevLevel != level)
	{
		...
		char advMsg[90];
		sprintf(advMsg, "Você voltou do level %d para o level %d.", prevLevel, level);
		sendTextMessage(MSG_EVENT_ADVANCE, advMsg);
	}
...
Item* Player::createCorpse(DeathList deathList)
{
	...
	std::stringstream ss;
	ss << "Você reconheceu " << getNameDescription() << ". " << (sex % 2 ? "Ele" : "Ela") << " Foi morta por ";
		...
		if(deathList[0].getKillerType() != deathList[1].getKillerType())
		{
			if(deathList[1].isCreatureKill())
			{
				...
				if(deathList[1].getKillerCreature()->getMaster())
					ss << " Criado por " << deathList[1].getKillerCreature()->getMaster()->getNameDescription();
			}
			else
...
void Player::onAddCombatCondition(ConditionType_t type, bool)
{
	...
	if(!tmp.empty())
		sendTextMessage(MSG_STATUS_DEFAULT, "Você está " + tmp + ".");
}
		...
		case MANAGER_NAMELOCK:
		{
			if(!talkState[1])
			{
				...
				if(managerString.length() < 4)
					msg << "O nome que você quer é muito pequeno, por favor coloque um maior.";
				else if(managerString.length() > 20)
					msg << "O nome que você quer é muito grande, por favor coloque um menor";
				else if(!isValidName(managerString))
					msg << "Esse nome parece conter caracteres inválidos, por favor coloque um diferente.";
				else if(IOLoginData::getInstance()->playerExists(managerString, true))
					msg << "Um jogador com esse nome já existe, por favor coloque um diferente.";
				else
				{
